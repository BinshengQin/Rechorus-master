...
#         # 初始化模型
#         self.model = LightGCNCpr(args, corpus)
#         self.optimizer = torch.optim.Adam(self.model.parameters(), lr=args.lr)
#         self.loss_fn = CPRLoss(self.model, beta=args.beta, gamma=args.gamma, k=args.k, reg=args.reg)

#     def train(self):
#         self.model.train()
#         for epoch in range(self.args.epochs):
#             total_loss = 0
#             for batch in self.model.sampler.sample(self.model):
#                 users, pos_items, neg_items = batch
#                 self.optimizer.zero_grad()
                
#                 # 计算损失
#                 loss = self.loss_fn(users, pos_items, neg_items)
#                 loss.backward()
#                 self.optimizer.step()
                
#                 total_loss += loss.item()

#             print(f"Epoch {epoch + 1}/{self.args.epochs}, Loss: {total_loss / len(self.corpus)}")

#     def evaluate(self):
#         self.model.eval()
#         # 评估代码，通常包括计算预测和实际之间的损失
#         pass
