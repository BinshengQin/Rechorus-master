...
class LGCNEncoder(nn.Module):
	...
	def forward(self, users, items):
		...
		# 	users = torch.tensor([users], dtype=torch.long, device='cuda') 
		# elif isinstance(users, list): 

		# 	users = torch.tensor(users, dtype=torch.long, device='cuda') 
		# 	# 确保 items 也处理类似 
		# if isinstance(items, int): items = torch.tensor([items], dtype=torch.long, device='cuda') 
		# elif isinstance(items, list): items = torch.tensor(items, dtype=torch.long, device='cuda')
		#if isinstance(users, int): device = 'cuda' 
		#sparse_norm_adj = self.sparse_norm_adj.to(device)			
